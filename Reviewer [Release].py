    ...
    def setQuestions(self,keys,qType):
        ...
        if qType == 'c':
            for i in keys:
                ...
                print('\n')
                try:
                    if let_choice.index(q) == choices.index(i):
                        self.tally_score(state_ans=True,ans_key=i)
                    else:
                        self.tally_score(state_ans = False,ans_key = i)
                except IndexError:
                    print('Input not available, marked as incorrect! ')
                    self.timed.sleep(10)
                    continue
                    
    def setReview(self,queType='',subject=None,textfile=None,num_items=None):
        ...
        num_item = len(self.mainKeys) if num_items == None else int(num_items)

        self.mainKeys = self.mainKeys[:num_item]
...
class mainRun(manageProcess,manageFiles):
    ...
    ----------------------------------------------------------
    SET commands:
    $start_rev-c ~ start choice-based review & list subjects
    $start_rev-d ~ start description-based review & list subjects
    $end ~ end application
    ...
    def interpret(self,inp_string):
        if inp_string[0] != '$':
            ...
            if inp_string[:-1] == '$start_rev-':
                print('Subjects created in this location')
                for floc in self.oss.listdir(self.subs):
                    print(floc)
                sub = input('What subject do you prefer to study right now? \n')
                print('Reviewers created in this subject')
                for files in self.oss.listdir(self.subs+'\\'+sub):
                    print(files)
                text = input('Is there any particular textfile that you would like to study? ')
                numItems = input('How many items do you wish to take? ')
                
                if sub == '':
                    sub = None
                    super().setReview(queType = str(inp_string[-1]),subject=sub,textfile=text+'.txt',num_items=numItems)
                elif text == '':
                    text = None
                    super().setReview(queType = str(inp_string[-1]),subject=sub,textfile=text,num_items=numItems)
                else:
                    super().setReview(queType = str(inp_string[-1]),subject=sub,textfile=text+'.txt',num_items=numItems)
